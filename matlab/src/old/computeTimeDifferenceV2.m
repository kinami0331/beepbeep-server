...
originChirpFill = [origin, zeros(1, length(record) - length(origin))]
[tau, R, lags] = gccphat(record, originChirpFill', 44100)
c = abs(R)
lags = lags'
...
fig = figure;
stem(lags(maxIndex-800:maxIndex+800), c(maxIndex-800:maxIndex+800));
...
fig = figure;
stem(lags(rangeStart2:rangeEnd2), c(rangeStart2:rangeEnd2))
